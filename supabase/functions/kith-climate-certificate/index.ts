...
async function handleSendEmail(body: SendEmailRequest) {
  ...
  try {
    const resp = await fetch(`${SUPABASE_URL}/functions/v1/kith-climate-send-email`, {
      ...
      body: JSON.stringify({
        to: cert.email,
        subject: `Congratulations, ${cert.first_name} — You're Kith Climate Certified`,
        html_body: emailHtml,
...
async function deployStaticCertificate(cert: any): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    const html = renderCertificateHtml(cert, cert.token);
    const filePath = `verify/${cert.certificate_number}/index.html`;
    const contentBase64 = base64Encode(new TextEncoder().encode(html));
    ...
    const payload: Record<string, unknown> = {
      message: `Add certificate ${cert.certificate_number} for ${cert.first_name} ${cert.last_name}`,
    ...
    if (!resp.ok) {
      const errBody = await resp.text();
      return { ok: false, error: `GitHub API ${resp.status}: ${errBody}` };
    }
...
<head>
  ...
  <title>${name} — Kith Climate Certified Professional</title>

  <!-- OpenGraph meta tags for LinkedIn sharing -->
  <meta property="og:title" content="${name} — Kith Climate Certified Professional" />
  ...
  <meta name="twitter:title" content="${name} — Kith Climate Certified Professional" />
...
<div class="certificate">
  ...
  <div class="content">
    ...
    <div class="middle-section">
      ...
      <div class="award-title">Kith Climate Certified Professional</div>
          ...
          <tr>
            <td align="center" style="padding:40px 40px 24px;">
              ...
              <h1 style="margin:0;font-size:26px;font-weight:600;color:#e8e6e3;letter-spacing:0.02em;">
                ${escapeHtml(cert.first_name)}, you did it.
              </h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:0 40px 32px;">
              <p style="margin:0 0 16px;font-size:16px;line-height:1.7;color:rgba(232,230,227,0.7);">
                You've officially completed the <strong style="color:#e8e6e3;">Kith Climate 8-Week Cohort</strong> and earned the title of <strong style="color:#5B9A8B;">Kith Climate Certified Professional</strong>.
              </p>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto;">
                <tr>
                  <td align="center" style="background:#5B9A8B;border-radius:8px;">
                    <a href="${certificateUrl}" target="_blank" style="display:inline-block;padding:14px 32px;font-size:16px;font-weight:600;color:#fff;text-decoration:none;letter-spacing:0.02em;">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:rgba(91,154,139,0.08);border-radius:8px;border:1px solid rgba(91,154,139,0.15);">
                <tr>
                  <td style="padding:20px 24px;">
                    ...
                    <p style="margin:0 0 4px;font-size:14px;color:rgba(232,230,227,0.7);">
                      <strong style="color:#e8e6e3;">Credential:</strong> Kith Climate Certified Professional
                    </p>
                    <p style="margin:0 0 4px;font-size:14px;color:rgba(232,230,227,0.7);">
                ...
                <tr>
                  <td style="padding:16px 20px;">
                    <p style="margin:0;font-size:13px;line-height:1.7;color:rgba(232,230,227,0.55);font-style:italic;">
                      ...
                      Over 8 weeks, I built tools for life cycle assessment, supply chain sustainability, carbon reduction, climate disclosure, circularity, and sustainability strategy. Not slides. Not quizzes. Working applications.<br><br>
          ...
          <tr>
            <td style="padding:0 40px 32px;">
              ...
              <p style="margin:0;font-size:14px;line-height:1.6;color:rgba(232,230,227,0.6);">
                The cohort is over, but the community isn't. Stay active in the Discord, share what you're building, and keep pushing the work forward. We'll be in touch about alumni opportunities as they develop.
              </p>
            </td>
          </tr>
